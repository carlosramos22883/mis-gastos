...
<div class="relative w-full">
    ...
    <input id="{{ $id }}_display" name="{{ $id }}_display" type="text" inputmode="numeric" maxlength="10" @required($required)
        ...
        {{ $attributes->except(['x-bind:name', 'name']) }} oninput="this.value = this.value.replace(/\D/g, '').slice(0, 8).replace(/^(\d{2})(\d)/, '$1/$2').replace(/^(\d{2}\/\d{2})(\d)/, '$1/$2')" onblur="validateDateInput(this)">
    <input id="{{ $id }}" name="{{ $id }}" @if($dynamicName) x-bind:name="{{ $dynamicName }}" @endif type="date" value="{{ $isoValue }}" min="{{ $min?->format('Y-m-d') }}" max="{{ ($max ?? now())->format('Y-m-d') }}"
    ...
    @endif
    <p data-date-error class="mt-1 hidden text-xs flex items-center gap-1 text-red-600 dark:text-red-400 font-medium"></p>
</div>
