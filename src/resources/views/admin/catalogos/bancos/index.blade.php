<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            @php
                $headers = [
                    ...
                    ['label' => 'Estado', 'key' => 'activo', 'sortable' => true, 'width' => 'w-24'],
                    ['label' => 'Acciones', 'sortable' => false, 'width' => 'w-32 text-right'],
                ];
            @endphp

            <x-data-table :headers="$headers" :data="$items" :createRoute="route('admin.catalogos.bancos.create')" createPermission="bancos.create"
                createModal="banco-modal" searchPlaceholder="Buscar por nombre o código..." defaultSort="nombre"
    ...
    <!-- Modal -->
    <x-modal name="banco-modal" :show="false" maxWidth="2xl">
        <div x-data="{ html: '', loading: false }" tabindex="-1"
            x-on:load-banco-modal-form.window="
            loading = true;
            html = '';
            fetch($event.detail)
                .then(r => r.text())
        ...
        "
            x-on:close-modal.window="if ($event.detail === 'banco-modal') { html = ''; loading = false; if ($refs.formContainer) $refs.formContainer.innerHTML = ''; }"
