<form x-data="ajaxForm(function() {
    $dispatch('close-modal', 'moneda-modal');
    showAlert('success', '¡Éxito!', 'Moneda guardada correctamente.');
    window.dispatchEvent(new CustomEvent('refresh-table'));
})" @submit.prevent="submit"
    action="{{ isset($moneda) ? route('admin.catalogos.monedas.update', $moneda) : route('admin.catalogos.monedas.store') }}"
    method="POST" novalidate class="space-y-4">
    @csrf
    @if (isset($moneda)) @method('PUT') @endif

    <div class="flex items-center justify-between mb-6">
        <h2 class="text-lg font-medium text-gray-900 dark:text-gray-100">{{ isset($moneda) ? 'Editar Moneda' : 'Nueva Moneda' }}</h2>
        <button type="button" x-on:click="$dispatch('close-modal', 'moneda-modal')" class="text-gray-400 hover:text-gray-500">
    ...
    <div class="flex items-center gap-2 mt-6">
        @php
            ...
            $isChecked = isset($moneda) ? $moneda->activo : old('activo', 1);
        @endphp
    ...
    <div class="mt-6 flex justify-end gap-3">
        <x-secondary-button type="button" @click="$dispatch('close-modal', 'moneda-modal')">Cancelar</x-secondary-button>
        <x-primary-button type="submit" x-bind:disabled="loading" class="relative">
            <span x-show="!loading">{{ isset($moneda) ? 'Guardar Cambios' : 'Crear Moneda' }}</span>
