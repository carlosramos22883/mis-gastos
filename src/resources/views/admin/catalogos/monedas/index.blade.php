<x-app-layout>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            @php                
                $headers = [
                    ...
                    ['label' => 'Código', 'key' => 'codigo', 'sortable' => true, 'width' => 'w-1/4'],
                    ['label' => 'Símbolo', 'key' => 'simbolo', 'sortable' => true, 'width' => 'w-1/4'],
                    ['label' => 'Estado', 'sortable' => false, 'width' => 'w-1/6'],
                    ['label' => 'Acciones', 'sortable' => false, 'width' => 'text-right'],
                ];
            @endphp

            <x-data-table :headers="$headers" :data="$items" 
                :createRoute="route('admin.catalogos.monedas.create')" 
                createPermission="monedas.create"
                createModal="moneda-modal" 
    ...
    <!-- Modal -->
    <x-modal name="moneda-modal" :show="false">
        <div x-data="{ html: '', loading: false }" tabindex="-1"
            x-on:load-moneda-modal-form.window="
            loading = true;
            html = '';
            fetch($event.detail)
                .then(r => r.text())
